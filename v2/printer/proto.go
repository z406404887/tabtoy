...
func (self *protoPrinter) Run(g *Globals) *Stream {
	...
	for _, d := range g.FileDescriptor.Descriptors {
		...
		if d.File.MatchServerForbidSheet(d.File.Name)&&d.Usage==model.DescriptorUsage_RowType{
			continue
		}

		if d.File.Name==g.Protooutputignorefile{
			continue
		}
		...
		for index, fd := range d.Fields {
			...
			if d.Usage == model.DescriptorUsage_CombineStruct {
				...
				if fd.Complex!=nil && fd.Complex.File.MatchServerForbidSheet(fd.Name){
					continue
				}
			}
